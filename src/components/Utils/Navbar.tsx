...
const Navbar = () => {
  ...
  const canAccess = (linkRoles?: string[]) => {
    if (!linkRoles) return true;
    if (!user?.tipo_usuario) return false;
    return linkRoles.includes(user.tipo_usuario);
  };
    ...
    <aside className="fixed left-0 top-0 h-screen w-60 bg-[#E6E9F2] border-r border-gray-300 shadow-lg flex flex-col justify-between z-40">
      <div>
        ...
        <ul className="flex flex-col mt-6 space-y-2 px-4">
          {navLinks
            .filter((link) => canAccess(link.roles))
      ...
      <ConfirmLogout open={confirmOpen} onConfirm={confirmLogout} onCancel={cancelLogout} />
    </aside>
  );
};
